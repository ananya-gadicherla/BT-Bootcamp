...
def generate_employee_report(employee):
    ...
    print("-"*50)
    print(f"{'Name':30} | {employee['name']}")
    print(f"{'Employee ID':30} | {employee['emp_id']}")
    print(f"{'Gross Monthly Salary':30} | ₹{employee['gross_monthly']:.2f}")
    print(f"{'Annual Gross Salary':30} | ₹{employee['annual_gross']:.2f}")
    print(f"{'Taxable Income':30} | ₹{employee['taxable_income']:.2f}")
    
    # Tax Breakdown
    print(f"{'Tax Payable (Breakdown)':30} |")
    ...
    print(f"{'Total Tax Payable':30} | ₹{employee['total_tax']:.2f}")
    print(f"{'Annual Net Salary':30} | ₹{employee['net_salary']:.2f}")
    print("="*50 + "\n")


# -----------------------------
# Example usage with previous calculations
# -----------------------------
# Simulated data after previous steps
employee_data = {
    'name': "John Doe",
    'emp_id': "E12345",
    'gross_monthly': 85000,
    ...
    'tax_breakdown': [
        ...
        (600001, 900000, 10, 30000),
        (900001, 970000, 15, 10500)
    ],
    'rebate': 0,
    'cess': 2200,
    'total_tax': 57600,
